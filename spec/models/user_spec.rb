...
RSpec.describe User, type: :model do
  ...
  context "a User can confirm their email" do
    it "can confirm its email address" do
      legit_email.user = no_email
      # allow(Shirt).to receive(:find).with("3").and_return(shirt)
      allow(Organization).to receive(:find_or_create_by).and_return(Organization.new(domain_name: legit_email.split('@')[1]))
      legit_email.email_activate
      expect(no_one.email_confirmed).to be_truthy
      expect(no_email.confirmed).to be_truthy
